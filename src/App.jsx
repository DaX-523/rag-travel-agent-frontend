...
function App() {
  ...
  const handleSendMessage = async () => {
    ...
    // Start timer for plane animation
    ...
    try {
      ...
      const response = await fetch(`${BASE_URL}/stream`, {
        ...
        body: JSON.stringify({
          prompt: userMessage,
          threadId: thread_id,
          userId: user?.id, // Include userId in request if available
        }),
      });
      ...
      const { threadId, response: assistantResponse } = responseData;
      ...
      if (assistantResponse) {
        ...
        simulateStreaming(assistantResponse, () => {
          // Add the complete message to messages array when streaming is done
          setMessages((prev) => [
            ...prev,
            {
  ...
  // Reload the active thread when token changes
  useEffect(() => {
    ...
    <AnimationProvider>
      <div className="h-screen w-screen flex bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a]">
        ...
        <div
          ...
          } absolute left-0 top-6 lg:top-0 h-full z-20`}
        >
          {/* App Logo */}
          ...
          <div className="px-3 pb-3">
            <button
              onClick={logout}
              className="w-full bg-white/5 hover:bg-white/10 text-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium transition-colors flex items-center justify-center gap-2"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
              ...
              >
                <div className="max-w-2xl mx-auto">
                  ...
                  <div className="space-y-6">
                    ...
                    {assistantMessageLoading && !streamingMessage && (
                      <div className="flex justify-start">
